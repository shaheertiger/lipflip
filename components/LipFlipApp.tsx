...
const AnnouncementBar = dynamic(() => import('./AnnouncementBar').then(m => m.AnnouncementBar), { ssr: false });
const StickyMobileCTA = dynamic(() => import('./StickyMobileCTA').then(m => m.StickyMobileCTA), { ssr: false });
const ActivityToast = dynamic(() => import('./ActivityToast').then(m => m.ActivityToast), { ssr: false });
const ComparisonTable = dynamic(() => import('./ComparisonTable').then(m => m.ComparisonTable), { ssr: false });
const MidPageCTA = dynamic(() => import('./MidPageCTA').then(m => m.MidPageCTA), { ssr: false });
...
const LipFlipApp: React.FC = () => {
  ...
  const executeGeneration = async () => {
    ...
    try {
      setStatus(ProcessingStatus.PROCESSING);
      setScanStep("Finalizing high-res output..."); // Keep showing overlay during actual generation
      setError(null);
      setIsCachedResult(false);
          ...
          {originalImage.previewUrl && processedImage && (
            <div className="space-y-8 md:space-y-10 animate-fade-in pt-4 md:pt-6">
              ...
              <div className="mt-20 text-center space-y-6 py-12 border-t border-white/5">
                ...
                <p className="text-slate-400 max-lg mx-auto px-4">
